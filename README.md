...
# 1. Install Python 3.12 venv package (If required on Ubuntu/Debian)
'''sudo apt install python3.12-venv'''

# 2. Clone the repository
git clone [https://github.com/samuelnaol72/f110-gym-py312.git](https://github.com/samuelnaol72/f110-gym-py312.git)
cd f110-gym-py312
...
Headless Mode: Initialize the environment with render_mode=None for server-side or non-visual use:

env = gym.make('f110_gym:f110-v0', render_mode=None, num_agents=1) 


Action Format: Actions must be passed as a NumPy array of shape (num_agents, 2). For a single agent, this is (1, 2).

Observation Conversion: The ROS Bridge must map the keys from the observation dictionary (obs) back to standard ROS messages (scans $\rightarrow$ LaserScan, poses_x/poses_y/poses_theta $\rightarrow$ PoseStamped/tf).
